            ...
            <View style={{
                ...
                {TableData.map((data,index) => {
                    return(
                        ...
                        >
                            <Text style={[{...styles.tbodyItem,flex:.6,paddingLeft:15,color:colors.title}, index % 2 === 0 && {color : COLORS.title}]}>{data.name}</Text>
                            <Text numberOfLines={1} style={[{...styles.tbodyItem,color:colors.title}, index % 2 === 0 && {color : COLORS.title}]}>{data.email}</Text>
                            <Text style={[{...styles.tbodyItem,flex:0.5,textAlign:'right',paddingRight:15,color:colors.title}, index % 2 === 0 && {color : COLORS.title}]}>{data.age}</Text>
                        </View>
                    )
                })}
            </View>
